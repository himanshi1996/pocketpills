...
export class AppComponent implements OnInit,AfterViewInit{
  ...
  ngAfterViewInit(): void {
    this.initiateIntersection(this.imageList);
  }
  /**
   * Initiate Intersection observer
   * @param itemList 
   */
  initiateIntersection(itemList){
    const images = document.querySelectorAll('img.image');
    images.forEach((element,i) => {
      element.setAttribute('data-src',itemList[i].url)
    });
  ...
  let options = {
       ...
       const imageObserver = new IntersectionObserver((entries : any, imgObserver) => {
  ...
  onNextClick() : void { 
    ...
    this.list = this.imageList.filter(el => el["isSelected"]==true);
    this.cd.detectChanges();
    this.initiateIntersection(this.list);
  }
  /**
